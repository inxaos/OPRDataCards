...
def find_fitting_font_size(text, font_name, base_font_size, min_font_size, max_width, max_height, line_height_mult=1.2):
    font_size = base_font_size
    while font_size >= min_font_size:
        lines = wrap_text_to_lines(text, font_name, font_size, max_width)
        total_height = len(lines) * int(font_size * line_height_mult)
...
def dataCardSpells(pdf, dataCardParameters, army):
    ...
    for armyId in casterArmyIds:
        ...
        for spell in armyRules['spells']:
            ...
            pdf.setFont("regular", fontSize)
            pdf.setFillColorRGB(0, 0, 0)
            # Dynamically fit text to one page
            rule_text = effect
            available_height = (startY + offsetY) - dataCardParameters["bottomClearance"]
            available_width = dataCardParameters["pdfSize"][0] - dataCardParameters["sideClearance"] - (startX + offsetXName)
            font_name = "regular"
            base_font_size = int(7 * 1.14)  # 14% larger than original 7 (7 * 1.14 ≈ 8)
            min_font_size = 5
            line_height_mult = 1.2
            font_size, lines = find_fitting_font_size(rule_text, font_name, base_font_size, min_font_size, available_width, available_height, line_height_mult)
            pdf.setFont(font_name, font_size)
            lineHeight = int(font_size * line_height_mult)
            for line in lines:
...
def dataCardRuleInfo(pdf, dataCardParameters, army):
    ...
    for rule in rules:
        for common in commonRules['rules']:
            if common['name'].lower() == "hero":
                continue
            if common['name'].lower() == "tough":
                continue
        ...
        for armyId in army.get('armyIds', []):
            ...
            for rule in rules:
                ...
                for armyRule in armyRules.get('specialRules', []):
                    if armyRule['name'].lower() == "hero":
                        continue
                    if armyRule['name'].lower() == "tough":
                        continue
